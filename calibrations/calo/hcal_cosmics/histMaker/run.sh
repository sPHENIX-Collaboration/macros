...
while read dir; do 
  ...
  for file in /sphenix/tg/tg01/commissioning/CaloCalibWG/hanpuj/cosmics-000"$dir"-*.prdf
  do
...
EOF
  ...
  do
    ...
    cat >>ff.sub<< EOF
+JobFlavour                   = "workday"
transfer_input_files          = ${WorkDir}/CondorRunJob$li.sh, ${WorkDir}/inputdata.txt,${WorkDir}/Fun4All_HCalCalib.C
...
done < runlist_clock.txt # redirect the input of the
