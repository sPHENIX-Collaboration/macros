...
int LiteCaloEval::process_event(PHCompositeNode *topNode)
{
  ...
  */
  ...
  if (m_UseTowerInfo)
  {
    ...
    if ( _ievent < 4 )
      std::cout << " Getting " << towernode << std::endl;
...
void LiteCaloEval::Get_Histos(const char *infile, const char *outfile)
{
  ...
  if (!(outF == ""))
  {
    if (m_myminbin == m_mymaxbin-1)
      {
	loc_file = new TFile(outfile,"RECREATE");
  ...
  for (int i = 0; i < max_ieta+1; i++)
  {
    ...
    if (loc_file && ( i == m_myminbin || i == 96 ) ) 
      eta_hist[i]->SetDirectory(loc_file);
    ...
    for (int j = 0; j < max_iphi; j++)
    {
      ...
      if (loc_file && i == m_myminbin) 
	cemc_hist_eta_phi[i][j]->SetDirectory(loc_file);
...
void LiteCaloEval::FitRelativeShifts(LiteCaloEval *ref_lce, int modeFitShifts)
{
  ...
  if (modeFitShifts % 1000 / 100 > 0)
  {
    flag_fit_rings = true;
    if (modeFitShifts % 1000 / 100 > 1)
    useRefFlat = true;  // this mode does the "flattening" 
    // in eta rings but using another reference file for 
    // the eta integrated spectra to use as the reference
    // it therefore does not subtract out the phi bin being fit.
  }
  ...
  else
  {
    if (calotype == LiteCaloEval::CEMC)
    {
      corrPat->SetMinimum(0.5);
      corrPat->SetMaximum(2.0);
    }
    else
    {
 ...
 if (fitalleta)
    {
      std::cout << "running fitalleta mode "  << ref_lce->eta_hist[96]  << std::endl;
      //flattening across eta, reference is total of whole detector
      // this will need changed to remove some etas/phis, etc...
      /// for emcal
      int iik = 96;
    ...
    TString cleanEta = "cleanEtaRef_";
      */
      hnewf = (TH1F *) ref_lce->eta_hist[iik]->Clone(myClnm.Data());
      std::cout << "hnewf " << hnewf << std::endl;
      hnewf->Rebin(5*200);
      std::cout << "hnewf " << hnewf << std::endl;
    }
  ...
  for (int i = minbin; i < maxbin; i++)
  {
	...
	if (useRefFlat)
	  cleanEtaRef = (TH1F *)  ref_lce->eta_hist[iik]->Clone(cleanEta.Data());
	else 
	  cleanEtaRef = (TH1F *) eta_hist[iik]->Clone(cleanEta.Data());
        // 4/1/23
        /*
        if( (i >= 2 && i<=93) )
          {
      ...
      {
	std::cout << "cleanetaref "  << iik << " " << ref_lce->eta_hist[iik]  << std::endl;

        hnewf = (TH1F *) ref_lce->eta_hist[iik]->Clone(myClnm.Data());
        hnewf->Rebin(5);
	std::cout << "hnewf"  << iik << " " << hnewf  << std::endl;
    ...
    else if (calotype == LiteCaloEval::HCALOUT)
    {
      if (flag_fit_rings == true)
      {
        ...
        cleanEta += iik;
        cleanEtaRef = (TH1F *) hcalout_eta[iik]->Clone(cleanEta.Data());
    ...
    else if (calotype == LiteCaloEval::HCALIN)
    {
      ...
      {
        cleanEta += iik;
        cleanEtaRef = (TH1F *) hcalin_eta[iik]->Clone(cleanEta.Data());
    ...
    if (flag_fit_rings == true)
    {
      ...
      {
      if (!(fitalleta && i > minbin))
	{
	  std::cout << "smoothing reference " << hnewf << std::endl;
    ...
    for (int j = 0; j < max_iphi; j++)
    {
      ...
      */
      ...
      local_grT->SetName(myClnm2p.Data());
  ...
  if (calotype == LiteCaloEval::CEMC && fitalleta == false)
  {
...
void LiteCaloEval::drawSpectraEmc(int flag)
{
  ...
  /// histo to hold the returned fit parameter value
  TH2F *corrPatChk = new TH2F("corrPatChk", "", 96, 0, 96, 256, 0, 256);
  ...
  for (int i = 0; i< 4; i++)
    {
      ...
      c1[i] = new TCanvas(c1nm[i].Data(),c1nm[i].Data(),900,600);
      c1[i]->Divide(2);
	...
	{ 
	  ...
	  int ieta = i*24+lr*12+jj;

	  if (! eta_hist[ieta])
	    continue;
	  //	  eta_hist[ieta]->Scale(1./256.0);
	  eta_hist[ieta]->Rebin(20);
	      //	      eta_hist[ieta]->Scale(1./256.0);
	  float scalebseta = eta_hist[ieta]->GetBinContent(4);
	  
	  float scalebs = pow(10,12-jj);
	  
	  if (scalebseta)
	    eta_hist[ieta]->Scale(scalebs/scalebseta);
	  ...
	  TH1F * h1start = (TH1F * ) cemc_hist_eta_phi[(2*i+lr)*12+jj][kk];
	  if (!h1start)
	    {
	      std::cout << "one hist 0 is null " << std::endl;
	      return; 
	    }

	  h1start->Rebin(20);

	  float fbinfrst = h1start->GetBinContent(4);


	  if (fbinfrst ==0 && jj == 0 && kk == 0)
	    h1start->SetBinContent(20,scalebs);
	  else if (fbinfrst)
	    h1start->Scale(scalebs/fbinfrst);


	  if (doRat) 
	    {
	      // future idea: fill a 2D histo , ->Fill(rat_pt point, pt point)
	      //redo iteration
	      int countbad = 0;
	      for (int jra = 4; jra < 20; jra++)
		{
		  float denometa = eta_hist[ieta]->GetBinContent(jra);
		  float numh1s = h1start->GetBinContent(jra);
		  if (!(denometa > 0) || !(numh1s > 0))
		    countbad++;
		  else if (fabs(numh1s/denometa - 1.0) > 0.2)
		    countbad++;
		}

	      if (countbad > 5)
		corrPatChk->Fill(ieta+1-0.5,kk+1-0.5);

	      
	      float hiEint =  0;
	      
	      for (int ibx = h1start->FindBin(6.0); ibx < h1start->GetNbinsX()+1; ibx++)
		{
		  hiEint +=  fbinfrst*(h1start->GetBinContent(ibx))/scalebs;
		}
	  ...
	  if (jj == 0 && kk == 0)
	    {
	      ...
	      h1start->SetMaximum(1e13);
	      h1start->SetMinimum(1e-2);
	      h1start->GetXaxis()->SetRangeUser(0.,1.5);
	    ...
	    {
	      if (corrPatChk->GetBinContent(ieta+1,kk+1) > 0 )
		h1start->SetLineColor(kMagenta);
	      //	      if (corrPatChk->GetBinContent(ieta+1,kk+1) > 0 )
		h1start->Draw("same");
	    }
  ...
  std::cout << "found this many bads : " << corrPatChk->GetEntries() << std::endl;  

}


void LiteCaloEval::drawSpectraEmcOneSlice(int flag, int ieta)
{
	...
	{ 
	  ...
	  if (lr < 1)
	    {
	      ...
	      eta_hist[ieta]->Rebin(20);
	      //	      eta_hist[ieta]->Scale(1./256.0);
	      scalebs= eta_hist[ieta]->GetBinContent(3);

	    }



	  std::cout << "rebinning eta_hist"  << eta_hist[ieta]->GetNbinsX() << std::endl;
	  ...
	  for (int kk  = 0; kk < 256; kk++)
	    {						
	      ...
	      //	      float scalebs = pow(10,0); // ie 1
	  
	  TH1F * h1start = (TH1F * ) cemc_hist_eta_phi[ieta][kk];
	  if (!h1start)
	    {
	      std::cout << "one hist 0 is null " << std::endl;
	      return; 
	  ...
	  if (lr > 0)
	    {
	      h1start = (TH1F * ) h1start->Clone();
	      h1start->Divide(eta_hist[ieta]);
	    }
	  
	  else
	    {
	      h1start->Rebin(20);
	      fbinfrst = h1start->GetBinContent(3);
	      if (fbinfrst)
		h1start->Scale(scalebs/fbinfrst);
	      h1start->GetBinContent(3);
	  ...
	  /*
	  if (fbinfrst ==0 && kk == 0)
	    h1start->SetBinContent(5,scalebs);
	  else 
	  */
	  ...
	  if (kk == 0)
	    {
	      ...
	      if (lr == 0)
		{
		  h1start->SetMaximum(scalebs*1000.);
		  h1start->SetMinimum(1e-3);
		  h1start->GetXaxis()->SetRangeUser(0.,1.5);
	  ...
	  eta_hist[ieta]->SetLineColor(2);
	  eta_hist[ieta]->Draw("Same");
