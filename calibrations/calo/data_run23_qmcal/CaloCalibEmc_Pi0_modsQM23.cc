...
void CaloCalibEmc_Pi0::Loop(int nevts, TString _filename, TTree * intree, const char * incorrFile)
{
  ...
  if (!(inF == ""))
  {
		...
		for (int ij = 0; ij < ntCorrs; ij++)
		{
			...
			/*
			if (ci == 40)
			  myaggcorr[ci][cj] = 0.170/0.180;
			else if (ci == 38)
			  myaggcorr[ci][cj] = 0.170/0.178;
			else if (ci == 31)
			  myaggcorr[ci][cj] = 0.170/0.185;
			else if (ci >8 && ci < 17 && ci != 13 && ci != 15)
			  myaggcorr[ci][cj] = 0.170/0.180;
			else if (ci == 68)
			  myaggcorr[ci][cj] = 0.170/0.180;
			else if (ci > 73 && ci < 77)
  ...
  for (int i = 0; i < nevts2; i++)
  {
    ...
    if ((i % 10 == 0 && i < 200) || (i%100 == 0 && i < 1000)
      ...
      std::cout << "recut evt no " << i << std::endl;
    ...
    for (int jCs = 0; jCs < iCs; jCs++)
    {
      ...
      // if (fabs(pho1->Pt()) < 2.5  && fabs(pho1->Eta()) > 0.7 )	
      // 				continue;
      ...
      // else
      // 	{
      // 	  pt1cut = 3.5 +  1.4*(iCs-90)/200.0;
      // 	  pt2cut = 2.0 +  1.4*(iCs-90)/200.0;
      // 	}
     ...
     // if (iCs < 50) 
     // 	{
     // 	  pt1cut =  2.65*modCutFactor;
     // 	  pt2cut  = 1.4*modCutFactor;
	  
     // 	}
     //  else
     // 	{
     // 	  pt1cut = 2.65*modCutFactor +  1.4*(iCs-49)/200.0*modCutFactor;
     // 	  pt2cut = 1.4*modCutFactor +  1.4*(iCs-49)/200.0*modCutFactor;
     // 	}
     ...
     // if (iCs < 30) 
     // 	{
     // 	  pt1cut =  1.65*0.170/.181;
     // 	  pt2cut  = 0.8*0.170/.181;
	  
     // 	}
     //  else
     // 	{
     // 	  pt1cut = 1.65*0.170/.181+  1.4*(iCs-29)/200.0;
     // 	  pt2cut = 0.8*0.170/.181 +  1.4*(iCs-29)/200.0;
     // 	}
	...
	if (fabs(pho2->Pt()) < pt2cut) continue;
				...
				if (alphaCut > 0.6) continue;
	
				TLorentzVector pi0lv;

        if (pho1->DeltaR(*pho2) > 1.1) continue;

        pi0lv = *pho1 + *pho2;
	if (pho1->E()  > 1.6 && pho2->E() > 0.75 && fabs(pi0lv.Pt()) > 1.18*(pt1cut+pt2cut))
	  		{
					float pairInvMass = pi0lv.M();
...
void CaloCalibEmc_Pi0::Fit_Histos_Etas96(const char * incorrFile)
{
  ...
  for (int iijk = 9; iijk < 96; iijk++)
	...
	{
	  nt_corrVals->Fill(iijk,jj,0.165/eta_par1_value[iijk],0.165/eta_par1_value[iijk]*myaggcorr[iijk][jj]);
	}
...
/*
// _______________________________________________________________..
void CaloCalibEmc_Pi0::Fit_Histos_Eta(const char * incorrFile)
{
  ...
  TNtuple * nt_corrVals = new TNtuple("nt_corrVals", "Ntuple of the corrections", "tower_eta:tower_phi:corr_val:agg_cv");
	...
	for (int ieta=0; ieta<96; ieta++)	// eta loop
	{
			...
			for (int kfi=1; kfi<20; kfi++)	// old kfi<25
			{
				float locbv = eta_hist[ieta]->GetBinContent(kfi);
				if (locbv > bsavloc)
				{
					pkloc = eta_hist[ieta]->GetBinCenter(kfi);
					bsavloc = locbv;
				}
			}
			...
			eta_hist[ieta]->Fit(f1[kj], "", "", pkloc-0.04, pkloc+0.04);
			float fpkloc2 = f1[kj]->GetParameter(1);


			TGraphErrors *grtemp = new TGraphErrors();
			TString bkgNm;
			bkgNm.Form("grBkgEta_phi_%d_%d", ieta, 0);
				
			std::cout << " getting eta slice histo " << bkgNm.Data() << " mean was " << fpkloc2\
			<< " " << pkloc << std::endl;

			grtemp->SetName(bkgNm.Data());
			int ingr = 0;
			for (int gj=1; gj<eta_hist[ieta]->GetNbinsX()+1; gj++)
			{
				float binc = eta_hist[ieta]->GetBinCenter(gj);
				float cntc = eta_hist[ieta]->GetBinContent(gj);
				if ((binc>0.06*fpkloc2/0.145 && binc<0.09*fpkloc2/0.145) || (binc>0.22*fpkloc2/0.145 && binc<0.35*fpkloc2/0.145))
				{
					grtemp->SetPoint(ingr,binc,cntc);
					grtemp->SetPointError(ingr++,0.001,sqrt(cntc));
			...
			eta_hist[ieta]->Fit(total[kj], "R");
			fit_fn[kj] = eta_hist[ieta]->GetFunction("total");
			...
			for (int jj =0; jj < 256; jj++)
			  {
			    nt_corrVals->Fill(ieta,jj,0.135/cemc_par1_values[ieta][iphi],0.135/cemc_par1_values[ieta][iphi]*myaggcorr[ieta][iphi]);
			  }
