...
//  std::string OutFile = "CALOHIST_" + filename;
  ...
  LiteCaloEval *eval7e = new LiteCaloEval("EVALUATOR", "HCALOUT",std::format("tsc_hcalout_{}.root",runnumber));
  eval7e->CaloType(LiteCaloEval::HCALOUT);
  eval7e->setInputTowerNodeName("TOWERINFO_CALIB_HCALOUT");
  se->registerSubsystem(eval7e);

  LiteCaloEval *eval7e2 = new LiteCaloEval("EVALUATOR2", "HCALIN",std::format("tsc_hcalin_{}.root",runnumber));
  ...
  std::cout << "adding validation module" << std::endl;
  CaloValid *ca = new CaloValid("calomodulename");
