...
GL1_FORMAT="/sphenix/lustre01/sphnxpro/physics/GL1/%s/GL1_%s_gl1daq-%08d-*"

GL1_LIST="${PWD}/list/run_%08d_gl1.list"

INTT_FORMAT="" # set based on run number in if statement later on
INTT_LIST="${PWD}/list/run_%08d_intt%01d.list"
...
HITMAP_FORMAT="${PWD}/hitmap/hitmap_run_%08d.root"
...
# File list creation
# GL1
printf -v GL1LIST ${GL1_LIST} ${RUN_NUM}
printf -v GL1FILE ${GL1_FORMAT} ${RUN_TYPE} ${RUN_TYPE} ${RUN_NUM} 
/bin/ls -1 ${GL1FILE} > ${GL1LIST} 2>/dev/null 
if [ ! -s ${GL1LIST} ]
then
...
# Macro
root -q -b "${PWD}/macro_Calib.C(${RUN_NUM}, ${NUM_EVT}, \"${GL1_LIST}\", \"${INTT_LIST}\", \"${HOTMAP_CDB_FILE}\", \"${HOTMAP_PNG_FILE}\", \"${BCOMAP_CDB_FILE}\", \"${BCOMAP_PNG_FILE}\")"
