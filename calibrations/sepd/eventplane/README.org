...
3. *Validation*: Applies the full correction (re-centering + flattening) to verify the final event plane resolution.
...
- *Output*: Generates flat TTrees for fast iteration and QA histograms to identify "bad" towers.
- *Condor*: Submits batch jobs to the HTCondor system for high-throughput processing.

*** 2. Stage-QVecCalib (Iterative Calibration)
- *Process*: Iteratively executes the three calibration passes using the ~GenQVecCalib~ binary.
...
*** 3. Stage-QVecCDB (CDB Generation)
- *Process*: Uses the ~GenQVecCDB~ binary to transform final results into sPHENIX CDB payloads.
- *Payloads*: Produces ~SEPD_EventPlaneCalib~ (moments and matrices) and ~SEPD_HotMap~ (bad tower statuses).
...
***  C++ Classes (C++20)
- *sEPD_TreeGen*: A ~SubsysReco~ module that extracts event-level and tower-level info into ROOT TTrees.
- *QVecCalib*: Orchestrates the physics logic for the three calibration passes.
- *QVecCDB*: Handles the formatting and commitment of data to the Calibration Database.
...
** Requirements & Build
- *Environment*: Must be run within the *sPHENIX environment* with ~OFFLINE_MAIN~ set.
...
- -i3, --dst-list-dir: Directory containing existing DST lists. If not provided, it defaults to a ~dst-lists~ folder within the output directory.
...
- -i4, --bin-QVecCalib: Path to the GenQVecCalib executable. If not specified, the script searches the ~OFFLINE_MAIN/bin~ directory.
- -i5, --bin-QVecCDB: Path to the GenQVecCDB executable. If not specified, the script searches the ~OFFLINE_MAIN/bin~ directory.
