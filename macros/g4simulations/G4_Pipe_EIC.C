...
void PipeInit()
{
  BlackHoleGeometry::max_radius = std::max(BlackHoleGeometry::max_radius, PIPE::be_pipe_radius + PIPE::be_pipe_thickness);
  BlackHoleGeometry::max_z = std::max(BlackHoleGeometry::max_z, PIPE::be_pipe_length_plus);
  BlackHoleGeometry::min_z = std::min(BlackHoleGeometry::min_z, PIPE::be_pipe_length_neg);
}
            ...
            int verbosity = 0)
{
  ...
  static const double be_pipe_length = PIPE::be_pipe_length_plus - PIPE::be_pipe_length_neg;  // pipe length
  static const double be_pipe_center = 0.5 * (PIPE::be_pipe_length_plus + PIPE::be_pipe_length_neg);

  if (radius > PIPE::be_pipe_radius)
  {
    cout << "inconsistency: radius: " << radius
         << " larger than pipe inner radius: " << PIPE::be_pipe_radius << endl;
    gSystem->Exit(-1);
  }
  ...
  cyl->set_double_param("thickness", PIPE::be_pipe_radius);
  ...
  cyl->set_double_param("radius", PIPE::be_pipe_radius);
  cyl->set_int_param("lengthviarapidity", 0);
  cyl->set_double_param("length", be_pipe_length);
  cyl->set_double_param("place_z", be_pipe_center);
  cyl->set_string_param("material", "G4_Be");
  cyl->set_double_param("thickness", PIPE::be_pipe_thickness);
  cyl->SuperDetector("PIPE");
  cyl->OverlapCheck(overlapcheck);
  if (absorberactive) cyl->SetActive();
  g4Reco->registerSubsystem(cyl);

  radius = PIPE::be_pipe_radius + PIPE::be_pipe_thickness;
